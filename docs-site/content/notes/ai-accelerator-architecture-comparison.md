...
<Badge type="tip" text="NVIDIA GPU" /> <Badge type="tip" text="Groq LPU/TSP" /> <Badge type="tip" text="Tenstorrent Tensix" /> <Badge type="tip" text="Google TPU" /> <Badge type="info" text="比较研究" />
...
- 全部官方与论文来源：[资料目录](/sources/catalog)

## 术语表

| 术语 | 概念 |
| --- | --- |
| AI accelerator | 针对 ML tensor、graph、memory 与 communication 特征优化的计算设备/系统。 |
| DSA | Domain-Specific Architecture，为一类应用领域而不是单一固定 function 优化的 architecture。 |
| SIMT | GPU 以 logical thread 编程、以 warp 成组执行的并行模型。 |
| Functional slicing | Groq 将同类功能单元聚成 slice 的空间组织。 |
| Time-space schedule | 同时决定 operation 的执行时间和物理资源位置。 |
| Tensix mesh | 由 NoC 连接的 Tensix core/device grid。 |
| Circular buffer | Tenstorrent local SRAM 中连接 producer/consumer kernel 的 FIFO。 |
| TensorCore（Google） | TPU chip 内含 MXU、vector/scalar unit 的计算组织。 |
| Tensor Core（NVIDIA） | GPU SM 内的 matrix-multiply execution pipeline。 |
| MXU | TPU 的 Matrix Multiplication Unit。 |
| Systolic array | data 以规则 wavefront 在 PE array 中移动与复用的结构。 |
| Scratchpad | software/compiler 显式管理的片上 memory。 |
| HLO | XLA 的 High Level Operations IR。 |
| GSPMD | XLA 的 graph/tensor SPMD partitioning system。 |
| PJRT | framework 与 device implementation 之间的 uniform runtime API。 |
| ICI | TPU chip 间 interconnect。 |
| AFD | Attention–FFN Disaggregation，当前特指 Rubin GPU 与 Groq 3 LPX 的 layer 内协作。 |
| Request-level routing | 以完整请求为边界选择 accelerator pool。 |
| Fine-grained heterogeneous execution | 在 layer/token/op 边界跨 accelerator，需要明确 ABI、transport 与同步。 |
