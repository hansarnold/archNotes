...
<Badge type="tip" text="Tenstorrent Tensix" /> <Badge type="info" text="架构专论" />
...
```mermaid
flowchart LR
    D["Device DRAM / remote L1"] --> R["Reader kernel\nData-movement RISC-V"]
    R --> CBI["Input circular buffers\nLocal SRAM"]
    CBI --> C["Compute kernel\nUnpack → FPU/SFPU → Pack"]
    C --> CBO["Output circular buffer\nLocal SRAM"]
    CBO --> W["Writer kernel\nData-movement RISC-V"]
    W --> O["Device DRAM / next core"]
```
...
```mermaid
flowchart TD
    F["PyTorch / JAX / ONNX / model frontends"] --> Forge["TT-Forge"]
    Forge --> MLIR["TT-MLIR\nTTIR / TTNN / D2M / TTKernel / TTMetal"]
    MLIR --> NN["TT-NN op library"]
    NN --> Metal["TT-Metalium runtime + kernels"]
    Metal --> HW["Tensix cores / SRAM / NoC / DRAM / mesh"]
```
