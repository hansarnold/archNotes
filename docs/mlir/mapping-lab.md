...
属于 [12 小时路线](./bootcamp.md)的两个单元：**单元 5 用 90 分钟读到“单元 5 验收”；完成 C++ B 与真实项目后，回来用 60 分钟完成单元 8。** 本页数字来自仓库中的分析脚本，不是某款 GPU/BPU 的实测。

## 单元 5：计算设备不是无限大的矩阵计算器
...
下一步是 [C++ B：实现微型 Pass](./cpp-labs.md)，随后读 [真实项目](./real-world.md)，再回到下半页。
